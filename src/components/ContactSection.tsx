...
const ContactSection = ({ showInfoCards: _showInfoCards = true }: { showInfoCards?: boolean }) => {
  ...
  const isInView = useInView(sidebarRef, { once: true, margin: "-60px" });
              ...
              <div className="rounded-2xl sm:rounded-3xl p-6 sm:p-8 bg-[#FAFAF8]">
                ...
                {submitted ? (
                  ...
                  <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-5">
                    ...
                    >
                      <div>
                        ...
                        <input required type="text" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })}
                          className="w-full px-4 py-3 rounded-xl bg-white text-gray-900 placeholder:text-gray-400 focus:outline-none text-sm transition-all duration-200"
                    ...
                    >
                      <button type="submit"
                        className="w-full py-4 rounded-xl font-bold text-sm flex items-center justify-center gap-2 active:scale-[0.98] hover:scale-[1.01] transition-all duration-200 text-white"
                        style={{
                          background: "linear-gradient(135deg, #00C2A8 0%, #09285A 100%)",
                          boxShadow: "0 8px 24px -4px rgba(0,194,168,0.4), 0 0 0 1px rgba(0,194,168,0.2), inset 0 1px 0 rgba(255,255,255,0.15)",
                        }}>
